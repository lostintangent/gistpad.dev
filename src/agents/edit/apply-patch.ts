...
class Parser {
    ...
    private parse_update_file(text: string): PatchAction {
        ...
        while (
            ...
            // Consume a hunk delimiter. A delimiter is any line that starts
            // with "@@", optionally followed by whitespace and an optional
            // context snapshot. If the delimiter carries no context, `defStr`
            // will be an empty string, and the parser will rely solely on the
            // subsequent chunk context for alignment.
            let defStr = "";
            if (this.startswith("@@")) {
                // `read_str("@@")` returns everything after the "@@" prefix and
                // advances `this.index`.
                defStr = this.read_str("@@").trimStart();
            } else if (index !== 0) {
                // Every chunk after the first must begin with a "@@" delimiter.
